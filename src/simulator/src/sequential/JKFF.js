 ...
 * @class
 * JKFF
 * Configurable JK flip flop with labeled ports (74-series style).
 * J=K=1: toggle. J=1,K=0: set. J=0,K=1: reset. J=K=0: hold.
 * @extends CircuitElement
 * @category sequential
 */
export default class JKFF extends CircuitElement {
    constructor(
        ...
        // J, CLK, K on left; EN on left edge 2 grids from bottom when active
        this.jInp = new Node(-30, -20, 0, this, 1, 'J')
    ...
    _commitOutputs() {
        ...
        if (!en && this.outputType === 'tristate') {
            ...
            qVal = this.state; qnVal = this.state ^ 1
        }
    ...
    resolve() {
        ...
        // Master-slave: sample inputs into masterState while clock is
        // inactive, transfer masterState → state on the active edge.
        // This prevents race conditions when multiple FFs share a clock.
        if (this.clkInp.value !== activeEdge) {
            // Clock inactive — sample J/K into masterState
            if (this._isEnabled()) {
                const j = this.jInp.value
                const k = this.kInp.value
            ...
            // Active edge — transfer master to slave (output)
            if (this._isEnabled()) {
    ...
    customSave() {
        return {
            nodes: {
                jInp: findNode(this.jInp),
                clkInp: findNode(this.clkInp),
                kInp: findNode(this.kInp),
    ...
    customDraw() {
        ...
        fillText3(ctx, 'J', -25, -20, xx, yy, 14, 'Raleway', 'left')
        fillText3(ctx, 'K', -25, 20, xx, yy, 14, 'Raleway', 'left')
...
JKFF.prototype.tooltipText = 'JK Flip Flop: J=K=1 toggles, J=1 sets, K=1 resets, J=K=0 holds'
JKFF.prototype.objectType = 'JKFF'
JKFF.prototype.helplink = 'https://docs.circuitverse.org/chapter4/chapter4-sequentialelements/#jk-flip-flop'

JKFF.prototype.mutableProperties = {
    resetType: {
        name: 'Reset Type',
        type: 'select',
